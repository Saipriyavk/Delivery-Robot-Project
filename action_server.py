...
class RobotActionsServer:
    ...
    def generate_init_state(self):
        ...
        state['traffic_stop'] = {'x':1.0, 'y': 3.0, 'orientation': "SOUTH"}
    ...
    def is_terminal_state(self, state):
        ...
        for key in state.keys():
            ...
            if key.startswith('robot'):
                x_coord = state[key]['x']
                y_coord = state[key]['y']
                pair = [x_coord, y_coord]
                coord = coord + pair
            for book in self.object_dict['books']:
                # print(self.object_dict['books'][book]['load_loc'])
                if coord in self.object_dict['books'][book]['load_loc'] and state['basket'] is not 'empty':
                    return True
        return False
