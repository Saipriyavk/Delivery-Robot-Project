...
class QLearning:
	...
	self.action_file = root_path + "/action_config.json"
	self.action_config = json.load(open(self.action_file))
    ...
    def qlearning(self, episodes):
		...
		#print random_action
		if flag is True:
		    r = self.action_config[random_action]['success_reward']
		    #print r
		else:
		    r = self.action_config[random_action]['fail_reward']
		    #print r
			
                # calculating cumulative reward
                intermediate_reward = math.pow(gamma, steps)*r
		
                reward = reward + intermediate_reward
		#print reward
		#print 'reward for cum', reward
                # updating q values
